...
// Polls left mouse-button clicks on an OpenCV window via
// cv::setMouseCallback. poll_click() drains the last buffered click, so a
// click is reported to exactly one caller.
class OpenCvInputSource : public IInputSource {
public:
    // `window_name` must match the name Renderer::draw() passes to
    // cv::imshow(), or clicks on that window won't reach this callback.
