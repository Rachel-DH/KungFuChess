...
// Turns (frame_count, frames_per_sec, is_loop) plus accumulated elapsed
// time into "which frame index to show" and "has a one-shot animation
// finished" — a generic sprite-sheet cycling primitive.
...
public:
    ...
    // Advances the animation by elapsed_ms, consuming whole frames as they
    // elapse and carrying over any leftover time (no drift across many
    // small calls). A no-op once finished().
